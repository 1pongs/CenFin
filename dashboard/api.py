...
def analytics_data(request):
    ...
    qs = (Transaction.objects
          .filter(user=request.user, date__range=[start, end], parent_transfer__isnull=True)
          .select_related('currency', 'account_destination__currency'))
    from django.db.models import Q
    if ent_ids:
        qs = qs.filter(Q(entity_source_id__in=ent_ids) | Q(entity_destination_id__in=ent_ids))
    if account_id:
        qs = qs.filter(Q(account_source_id=account_id) | Q(account_destination_id=account_id))
    ...
    if dimension == 'categories':
        ...
        for tx in qs:
            ...
            if (tx.transaction_type_source or '').lower() == 'expense':
                amt = convert_to_base(tx.amount or Decimal('0'), tx.currency, base_cur, user=request.user)
                for c in cats:
    ...
    # entities dimension
    from entities.models import Entity
    ent_names = {e.id: e.entity_name for e in Entity.objects.filter(Q(user=request.user) | Q(user__isnull=True))}
    ...
    for tx in qs:
        ...
        if (tx.transaction_type_source or '').lower() == 'expense' and tx.entity_source_id:
            exp[tx.entity_source_id] = exp.get(tx.entity_source_id, Decimal('0')) + convert_to_base(tx.amount or Decimal('0'), tx.currency, base_cur, user=request.user)
...
def monthly_audit(request):
    ...
    qs_initial = (Transaction.objects
                  .filter(q, date__lt=start_month)
                  .select_related('currency', 'account_destination__currency', 'account_destination', 'account_source'))

    def amt_in(tx):
        if (getattr(tx, 'destination_amount', None) is not None
            and getattr(tx, 'account_destination', None)
            and getattr(tx.account_destination, 'currency', None)):
            return convert_to_base(tx.destination_amount or Decimal('0'), tx.account_destination.currency, base_cur, user=request.user)
        return convert_to_base(tx.amount or Decimal('0'), tx.currency, base_cur, user=request.user)
    ...
    qs = (Transaction.objects
          .filter(q, date__range=[start_month, end])
          .select_related('currency', 'account_destination__currency', 'account_destination', 'account_source')
          .order_by('date', 'id'))
    ...
    for tx in qs:
        ...
        tx_rows.append({
            ...
            'account_destination': getattr(tx.account_destination, 'account_name', None),
            ...
            'amount_dest': float(tx.destination_amount or 0) if tx.destination_amount is not None else None,
            'currency_dest': getattr(getattr(tx, 'account_destination', None), 'currency', None).code if getattr(getattr(tx, 'account_destination', None), 'currency', None) else None,
    ...
    out_months = []
    for d in seq:
        row = months.get(d, {'income': Decimal('0'), 'expenses': Decimal('0'), 'liquid_delta': Decimal('0'), 'non_liquid_delta': Decimal('0')})
