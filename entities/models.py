...
class Entity(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self._state.adding:
            qs = Entity.objects.filter(
                entity_name__iexact=self.entity_name, user=self.user
            )
            ...
            if self.is_system_default:
                ...
                if (
                    orig.entity_name != self.entity_name
                    or orig.is_active != self.is_active
                ):
    ...
    def current_balance(self):
        ...
        base_qs_in = Transaction.objects.filter(
        ...
        inflow = base_qs_in.aggregate(
            total=Sum(
                Case(
                    When(
                        destination_amount__isnull=False, then=F("destination_amount")
                    ),
                    default=F("amount"),
        ...
        base_qs_out = Transaction.objects.filter(
            entity_source=self,
            asset_type_source__iexact="liquid",
            child_transfers__isnull=True,
            is_reversal=False,
        ).exclude(description__istartswith="reversal of")
        outflow = base_qs_out.aggregate(total=Sum("amount")).get("total") or Decimal(
            "0"
        )
