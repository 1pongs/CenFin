...
class LoanForm(forms.ModelForm):
    lender_id = forms.ModelChoiceField(
        queryset=Lender.objects.all(), widget=forms.HiddenInput(), required=False
    )
    lender_text = forms.CharField(label="Lender / Issuer", required=False)
    account_destination = forms.ModelChoiceField(
        queryset=Account.objects.none(), label="Account Destination"
    )
    account_source = forms.ModelChoiceField(
        queryset=Account.objects.none(), widget=forms.HiddenInput(), required=False
    )
    entity_source = forms.ModelChoiceField(
        queryset=Entity.objects.none(), widget=forms.HiddenInput(), required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Allow views to request disabling numeric edits (used on update)
        ...
        self.fields["lender_text"].widget.attrs.update(
            {"list": "lender-list", "autocomplete": "off"}
        )
        choices = [
        ...
        if user is not None:
            account_qs = Account.objects.filter(
                user=user, is_active=True, system_hidden=False
            ...
            self.fields["account_source"].queryset = Account.objects.filter(
                pk=outside_acc.pk
            )
            self.fields["account_source"].initial = outside_acc
            self.fields["entity_source"].queryset = Entity.objects.filter(
                pk=outside_ent.pk
            )
            self.fields["entity_source"].initial = outside_ent
            self.fields["entity_destination"].queryset = Entity.objects.filter(
                pk=account_ent.pk
            )
        ...
        # If requested, disable editing of principal_amount when updating
        if (
            getattr(self, "_disable_amount", False)
            and self.instance
            and self.instance.pk
        ):
            try:
                self.fields["principal_amount"].disabled = True
            except Exception:
                pass
    ...
    def clean(self):
        ...
        if lender:
            ...
            if not name:
                self.add_error(
                    "lender_text",
                    "Lender is required — select one or create a new lender first.",
                )
            else:
                self.instance.lender, _ = Lender.objects.get_or_create(
...
class CreditCardForm(forms.ModelForm):
    issuer_id = forms.ModelChoiceField(
        queryset=Lender.objects.all(), widget=forms.HiddenInput(), required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["issuer_text"].widget.attrs.update(
    ...
    def clean(self):
        ...
        if issuer:
            ...
            if not name:
                ...
                self.instance.issuer, _ = Lender.objects.get_or_create(
                    name__iexact=name, defaults={"name": name}
                )
        return cleaned_data
