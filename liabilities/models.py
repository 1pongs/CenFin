...
class Loan(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.term_months and self.received_date and self.maturity_date:
            self.term_months = (
                self.maturity_date.year - self.received_date.year
            ) * 12 + (self.maturity_date.month - self.received_date.month)
        if self.monthly_payment is None and self.term_months:
            self.monthly_payment = (
                self.principal_amount / Decimal(self.term_months)
    ...
    def delete(self, *args, **kwargs):
        ...
        tx_ids.extend(
            self.payments.filter(transaction_id__isnull=False).values_list(
...
class CreditCard(models.Model):
    ...
    issuer = models.ForeignKey(
        Lender, on_delete=models.PROTECT, related_name="credit_cards"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if update_fields and set(update_fields).issubset(
            {"outstanding_amount", "available_credit"}
        ):
