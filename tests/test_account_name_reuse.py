...
class AccountNameReuseTests(TestCase):
    ...
    def test_inactive_account_reused(self):
        ...
        self.assertEqual(Account.objects.filter(account_name="Woori Card", user=self.user, is_active=True).count(), 1)
    ...
    def test_soft_delete_then_readd_single_active(self):
        ...
        self.assertEqual(resp.status_code, 302)
        self.assertEqual(Account.objects.filter(account_name="Woori Card", user=self.user, is_active=True).count(), 1)
        self.assertEqual(Account.objects.filter(account_name="Woori Card", user=self.user).count(), 1)
