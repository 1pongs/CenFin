...
def _unit_members(unit: Transaction) -> List[Transaction]:
    ...
    """
    if unit.group_id:
        return list(Transaction.all_objects.filter(group_id=unit.group_id, is_deleted=False))
    return [unit]
    ...
    for t in members:
        ...
        for acc_id in accounts:
            last_seq = (
                ...
                .filter(Q(account_source_id=acc_id) | Q(account_destination_id=acc_id), is_deleted=False)
            ...
            if t.seq_account != last_seq:
                ...
                blockers.append(LedgerBlocker(account_id=acc_id, newer=list(newer.values("id", "seq_account", "description"))))
...
def reverse_unit(unit: Transaction, actor, reverse_date=None) -> List[Transaction]:
    ...
    with db_transaction.atomic():
        for t in members:
            ...
            t.save(update_fields=["is_reversed", "reversed_at", "reversed_by", "ledger_status"])
    return reversals
...
def delete_unit(unit: Transaction, mode: str, actor) -> List[Transaction]:
    ...
    reversals: List[Transaction] = []
    with db_transaction.atomic():
        if mode == "reverse_delete_unit" and unit.transaction_type not in {"income", "loan_disbursement"}:
            for m in members:
        ...
        for m in members:
            ...
            m.save(update_fields=["is_deleted", "deleted_at", "deleted_by", "ledger_status"])
    return reversals