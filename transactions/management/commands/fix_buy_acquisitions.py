...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user-id", type=int, default=None, help="Limit to a specific user id")
    ...
    def handle(self, *args, **options):
        ...
        qs = (
            ...
            .filter(q)
            .filter(Q(transaction_type__iexact="transfer") | Q(transaction_type__isnull=True))
            .select_related("account_destination", "entity_destination")
        )

        def is_outside_account(acc):
            return bool(acc and (getattr(acc, "account_type", None) == "Outside" or getattr(acc, "account_name", None) == "Outside"))
        ...
        for t in qs:
            ...
            if not t.entity_destination_id:
                continue
            ent_name = getattr(getattr(t, "entity_destination", None), "entity_name", "") or ""
            if ent_name.lower() == "outside":
                continue

            # Reclassify
            self.stdout.write(f"Reclassify tx #{t.id} -> buy acquisition (dest entity: {ent_name})")
            if not dry:
